...
class TestLoginForm(TestCase):
    ...
    def test_form_valid(self):
        data = {"username": "test@testing.com", "password": "1234Testing"}
        login_form = LoginForm(data=data)
        self.assertTrue(login_form.is_valid())

    def test_form_invalid_password(self):
        data = {"username": "test@testing.com", "password": "Testing"}
        login_form = LoginForm(data=data)
        self.assertFalse(login_form.is_valid())

    def test_form_invalid_email(self):
        data = {"username": "no_email@testing.com", "password": "testing123"}
        login_form = LoginForm(data=data)
        self.assertFalse(login_form.is_valid())
