...
class RegisteredFoodCreateForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        queryset = RegisteredFood.objects.filter(user_profile=user_profile ,food=food ,meal=meal)
        if queryset.exists():
            raise forms.ValidationError(_("User already have this food and meal registered."))
        return cleaned_data